...
cd /d "%~dp0.."
rem Stage outside the source tree: Node resolves require() by walking up parent
rem folders, so a stage inside source\ would silently borrow source\node_modules
rem and the check below could not catch a module missing from the install.
set "OUT=%TEMP%\CafeAttendance-stage"
...
set "CHECKDB=%OUT:\=/%"
set "DATABASE_URL=file:%CHECKDB%/check.db"
"%OUT%\node.exe" "%OUT%\app\migrate.js" || exit /b 1
del /q "%OUT%\check.db" 2>nul

echo [5/5] inno setup
"%ISCC%" /DStage="%OUT%" installer\setup.iss || exit /b 1
